...
public class Intake extends SubsystemBase {
  ...
  private Intake() {
    motor = new CANSparkMax(IntakeMap.MOTOR, MotorType.kBrushless);
...
interrupted -> {
      ...
      () -> {
        if(getOpen())
        {
          if(encoder.getPosition() >= IntakeMap.CLOSEANGLE-IntakeMap.BUFFER || encoder.getPosition() <= IntakeMap.CLOSEANGLE+IntakeMap.BUFFER)
          {
            return true;
          }
          else
        ...
        {
          if(encoder.getPosition() >= IntakeMap.OPENANGLE-IntakeMap.BUFFER || encoder.getPosition() <= IntakeMap.OPENANGLE+IntakeMap.BUFFER)
          {
            return true;
          }
          else
